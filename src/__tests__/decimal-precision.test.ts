...
test('minimum positive value preserves 7 decimal places', () => {
  ...
  assert.strictEqual(d.toString(), '0.0000001');
});
...
test('Decimal.mul avoids binary float error on large amounts', () => {
  ...
  assert.strictEqual(half.toString(), '5000000000.00000005');
});

test('JSON replacer serialises Decimal as exact string, not object', () => {
